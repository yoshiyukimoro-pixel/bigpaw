...
# Public list images use the lighter card variant; detail overrides the hero separately.
...
# Optimized media route. Only puppy-bound uploads are exposed; legacy files with no upload row
# follow the same visibility behavior as the existing /uploads path.
...
media_route="""        mm=re.fullmatch(r'/media/([^/]+)',path)
        if mm:
            ...
            width={'thumb':480,'card':720,'hero':1200}.get(kind,1200)
            safe_kind=kind if kind in ('thumb','card','hero') else 'hero'
...
# Public puppy-detail payload: hero-sized main + hero-sized gallery URLs.
...
print('PUPPY_IMAGE_DELIVERY_OK|hero=1200|card=720|thumb=480|format=webp|originals=preserved|public_only=guarded',flush=True)
