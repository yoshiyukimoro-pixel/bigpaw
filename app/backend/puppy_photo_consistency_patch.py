...
tag='<script src="assets/breeder-puppy-photo-normalize.js?v=20260926a"></script>'
if tag not in s:
    assert '</body>' in s,'breeder_puppy_body_missing'
    s=s.replace('</body>',tag+'</body>',1)
p.write_text(s,encoding='utf-8')
print('PUPPY_PHOTO_CONSISTENCY_OK|public_ratio=1x1|breeder_adjust=enabled|upload=client_square_compressed',flush=True)
