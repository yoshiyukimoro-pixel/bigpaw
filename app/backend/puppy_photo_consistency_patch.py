...
new='1枚目がメイン写真です。掲載枠は正方形です。「写真を調整」で元写真を見ながら、白い正方形枠に合わせて位置・拡大縮小をブリーダー様ご自身で決められます。JPG / PNG / WebP・各8MBまで'
if new not in s:
...
for old_tag in (
    ...
    s=s.replace(old_tag,'')
tag='<script src="assets/breeder-puppy-photo-normalize.js?v=20260926g"></script>'
assert '</body>' in s,'breeder_puppy_body_missing'
s=s.replace('</body>',tag+'</body>',1)
p.write_text(s,encoding='utf-8')
print('PUPPY_PHOTO_CONSISTENCY_OK|square_frame=fixed|editor=crop_overlay|outside_crop=visible_dimmed|breeder_pinch_pan=enabled|zoom_slider=enabled|rotate=enabled|brightness_toggle=enabled|adjusted_square_saved=enabled|legacy_final_editor=removed|asset_version=20260926g',flush=True)
