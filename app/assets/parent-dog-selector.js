(()=>{
  ...
  function populate(select,sex,currentValue){
    const list=parents.filter(x=>String(x.sex||'')===sex),keep=String(currentValue??'').trim();
    select.innerHTML='';
    const ph=document.createElement('option');ph.value='';ph.textContent='選択してください';select.appendChild(ph);
    const unknown=document.createElement('option');unknown.value='未登録';unknown.textContent='未登録';select.appendChild(unknown);
    ...
    if(keep&&keep!=='未登録'&&!list.some(d=>String(d.name)===keep)){const legacy=document.createElement('option');legacy.value=keep;legacy.textContent=keep+'（既存登録）';select.appendChild(legacy)}
    select.value=keep;
    if(!select.value&&keep==='未登録')select.value='未登録';
  }
  const KANA_ROWS=[
    ['あ行','アイウエオァィゥェォヴ'],
    ['か行','カキクケコガギグゲゴ'],
  ...
  function installInputUnits(){[['weight','kg'],['adultMin','kg'],['adultMax','kg'],['price','円']].forEach(([id,unit])=>addUnitSuffix(id,unit))}
