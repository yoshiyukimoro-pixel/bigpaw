(()=>{
  ...
  function card(label,d,fallback){
    ...
    const photo=url?`<div class="bp-parent-photo"><img src="${esc(url)}" loading="lazy" decoding="async" alt="${esc(name)}" data-focal-x="${a.x}" data-focal-y="${a.y}" data-focal-z="${a.z}"></div>`:`<div class="bp-parent-photo bp-parent-empty">🐩<small>写真未登録</small></div>`;
  ...
  window.addEventListener('pageshow',e=>{if(e.persisted){window.__BIGPAW_DETAIL_PUPPY=null;setTimeout(run,0)}});window.addEventListener('resize',()=>applyLayouts(document));
})();
