...
RUN python3 -c "from pathlib import Path; tag='<script src=\\\"/auth-return-fix.js\\\"></script>'; files=['breeder-register.html','login.html','register.html','verify-email.html','admin.html','breeder-puppy-new.html','breeder-inquiries.html','breeder-billing.html','breeder-deal-report.html','operator-admin.html','operator-breeders.html','operator-breeder-applications.html','operator-listings.html','operator-deals.html','operator-support.html','operator-deal-reports.html','operator-revenue.html','operator-reports.html','operator-invoices.html','operator-automations.html','mypage.html','my-page.html','account.html','messages.html','notifications.html']; [(lambda p,s: p.write_text(s.replace('</body>',tag+'</body>') if tag not in s else s,encoding='utf-8'))(Path(x),Path(x).read_text(encoding='utf-8')) for x in files if Path(x).exists()]"
RUN python3 -c "from pathlib import Path; css='''<style id=\\\"bigpaw-role-theme\\\">body{background:#fff7fb!important}body.bigpaw-breeder-theme{background:#f3f8ff!important}body.bigpaw-operator-theme{background:#fffbea!important}</style><script>(function(){var p=location.pathname;if(p.indexOf(\\\"/operator-\\\")>=0)document.body.classList.add(\\\"bigpaw-operator-theme\\\");else if(p.indexOf(\\\"/breeder-\\\")>=0||p.endsWith(\\\"/admin.html\\\"))document.body.classList.add(\\\"bigpaw-breeder-theme\\\");})();</script>'''; files=['admin.html','breeder-puppy-new.html','breeder-inquiries.html','breeder-billing.html','breeder-deal-report.html','operator-admin.html','operator-breeders.html','operator-breeder-applications.html','operator-listings.html','operator-deals.html','operator-support.html','operator-deal-reports.html','operator-revenue.html','operator-reports.html','operator-invoices.html','operator-automations.html','mypage.html','my-page.html','account.html']; [(lambda p,s:p.write_text(s.replace('</body>',css+'</body>') if 'bigpaw-role-theme' not in s else s,encoding='utf-8'))(Path(x),Path(x).read_text(encoding='utf-8')) for x in files if Path(x).exists()]"
...
p=Path('backend/server.py'); s=p.read_text(encoding='utf-8')
needle="print(f'BIG PAW v1.0 server running on port {port} ({APP_ENV})')"
insert="""# Safe auth-role diagnostic.
try:
    if IS_PRODUCTION:
        _ac=db()
...
"""
assert needle in s
if 'AUTH_ROLE_DIAG|' not in s:s=s.replace(needle,insert+needle,1)
p.write_text(s,encoding='utf-8'); py_compile.compile(str(p),doraise=True)
print('AUTH_ROLE_DIAG_PATCH_OK')
PY
