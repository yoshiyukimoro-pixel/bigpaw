...
# Exact renderer inspection: "この子について" is generated by #bigpaw-detail-real-gallery-js as #bigpawDetailMore .detailsection p.
assert 'id="bigpawDetailMore"' in s and '<h2>この子について</h2><p>' in s
assert '#bigpawDetailMore' in s
# Target the actual generated paragraph directly.
...
# After dynamic rendering, force the exact first "この子について" paragraph to preserve newline characters.
...
p.write_text(s,encoding='utf-8');x=p.read_text(encoding='utf-8')
assert '#bigpawDetailMore .detailsection:first-child p{white-space:pre-wrap!important}' in x
assert 'bigpawAboutTextLinebreakRuntime' in x
srv=root/'backend/server.py';py_compile.compile(str(srv),doraise=True)
